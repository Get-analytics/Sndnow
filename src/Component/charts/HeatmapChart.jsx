...
const HeatmapChart = ({
  ...
  const getHotspotRadius = (intensity) => 20 + (intensity * 5);
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        >
          {[...Array(3)].map((_, i) => (
            <button key={i} className={`p-2 hover:bg-gray-100 ${i < 2 ? "border-r" : ""}`}>
              <svg className="w-4 h-4 text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path d={["M15 12a3 3 0 11-6 0 3 3 0 016 0zM2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z",
                          ...
                          "M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"][i]} strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} />
              </svg>
            </button>
          ))}
        </motion.div>
      ...
      <div className="mt-6 flex flex-col md:flex-row justify-between">
        {[
          { title: "Total Clicks", value: "3,834", sub: "+7.2% from last month" },
          { title: "Click Distribution", value: "CTR 4.7%", sub: "82% focused on CTA buttons" },
          { title: "Most Active Time", value: "Tue 2-4pm", sub: "54% of all interactions" }
        ].map((stat, i) => (
          <motion.div 
            key={i}
                ...
                <svg className="w-3 h-3 text-[#7C5832]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path d={
                    ["M13 7h8m0 0v8m0-8l-8 8-4-4-6 6",
                     ...
                     "M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"][i]
                  } strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} />
                </svg>
              </div>
              <h3 className="font-medium text-gray-800 text-sm">{stat.title}</h3>
            </div>
            <div className="text-2xl font-bold text-[#7C5832]">{stat.value}</div>
